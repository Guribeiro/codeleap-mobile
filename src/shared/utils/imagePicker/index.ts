...
export const requestMediaLibraryPermissions = async () => {
  try {
    if (Platform.OS !== 'web') {
      ...
      if (status !== 'granted') {
        throw new Error('Desculpe, não temos permissão de acesso às suas fotos')
      }
    }
  } catch (error) {
...
export const requestCameraPermissions = async () => {
  try {
    if (Platform.OS !== 'web') {
      ...
      if (status !== 'granted') {
        throw new Error('Desculpe, não temos permissão de acesso à sua câmera')
      }
    }
  } catch (error) {
    const err = error as Error
...
export const launchCamera = async ({
  options,
}: PickerOptions): Promise<ImagePickerResult> => {
