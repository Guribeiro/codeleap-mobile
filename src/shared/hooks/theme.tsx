...
interface ThemeContextData {
  changeTheme(themeName: ChangeThemeProps): void
...
const ThemeProvider = ({ children }: Props): JSX.Element => {
  ...
  const changeTheme = useCallback(
    async ({ themeName }: ChangeThemeProps) => {
      const themeOptions = {
        dark,
        light,
      }
  ...
  const changeMainColor = useCallback(
    async ({ color }: ChangeMainColorProps) => {
  ...
  return (
    <ThemeContext.Provider
      value={{ changeTheme, theme: customTheme, changeMainColor }}
    >
